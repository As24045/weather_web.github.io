...
function sub() {
    ...
    if (cityName != "") {
        const urlWithParams = `${apiUrl}?key=${apiKey}&q=${encodeURIComponent(cityName)}&days=14&aqi=no&alerts=no`
        ...
        async function fetchData() {
            try {
                ...
                dateDB.forEach(dat => {
                    ...
                    document.getElementById("future-day-forecast").innerHTML += `<div class="forecastData"><p>${dateArr[8]}${dateArr[9]}/${dateArr[5]}${dateArr[6]}</p> <img src="${dat.day.condition.icon}"
                     alt="" class="forImg"> <p>${Math.round(dat.day.mintemp_c)}\u00B0C</p><p>${Math.round(dat.day.maxtemp_c)}\u00B0C</p></div>`
                })
                ...
                filteredData.forEach(hour => {
                    document.getElementById("firstDay").innerHTML += ` <div class="firstHourData"> <p> ${hour.time[11]}${hour.time[12]}:${hour.time[14]}${hour.time[15]}</p> <img src="${hour.condition.icon}" class="firstImg"> <p> ${Math.round(hour.temp_c)}</p></div>`
                });
